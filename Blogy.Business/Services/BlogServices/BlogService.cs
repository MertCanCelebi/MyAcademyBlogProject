...
namespace Blogy.Business.Services.BlogServices
{
    public class BlogService(IBlogRepository _blogRepository, IMapper _mapper) : IBlogService
    {
        ...
        {
            var values = await _blogRepository.GetAllAsync(x => x.WriterId == writerId);
            return _mapper.Map<List<ResultBlogDto>>(values);
        }

        public async Task<List<ResultBlogDto>> GetBloksWithCategoriesAsync()
        {
            var values = await _blogRepository.GetBloksWithCategoriesAsync();
            return _mapper.Map<List<ResultBlogDto>>(values);
        }
