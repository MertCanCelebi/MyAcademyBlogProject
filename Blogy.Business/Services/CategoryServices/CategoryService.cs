...
namespace Blogy.Business.Services.CategoryServices
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<List<ResultCategoryWithBlogCountDto>> GetCategoriesWithBlogCountAsync()
        {
            var categories = await _categoryRepository.GetCategoriesWithBlogsAsync();
            var dtos = categories
                .Where(c => c.Blogs != null && c.Blogs.Any())
                .Select(c => new ResultCategoryWithBlogCountDto
                    {
                    Id = c.Id,
                    CategoryName = c.Name,
